    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {petNutritionMock.map((item) => {
          ...
          const daysLeft = calculateDaysRemaining(item.currentStock, item.dailyAmount);
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-lg font-bold text-white">{item.foodName}</h3>
                  <span className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest">
                    Pet: {getPetName(item.petId)}
                  </span>
                </div>
                <div className="text-right">
                  <span className="text-2xl font-bold font-mono text-emerald-500">{item.currentStock}kg</span>
                  <p className="text-[10px] text-zinc-500 uppercase tracking-tighter">Total: {item.maxStock}kg</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className={`p-3 rounded-md text-center border ${
                  daysLeft < 7 ? 'bg-rose-500/10 border-rose-500/30' : 'bg-zinc-800/50 border-zinc-800'
                }`}>
                  <p className={`text-xs font-medium uppercase tracking-widest ${daysLeft < 7 ? 'text-rose-500' : 'text-zinc-400'}`}>
                    Duração Estimada: <span className="font-bold text-white">{daysLeft} dias</span>
                  </p>
                </div>
              </div>
            </div>

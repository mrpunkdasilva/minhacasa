...
import { ArrowLeft, Calendar, Tag, DollarSign, FileText, Repeat } from "lucide-react";
...
export default async function InvoiceDetailView({ uuid }: InvoiceDetailViewProps) {
  const invoice = await getInvoiceByUuid(uuid);

  if (!invoice) {
    return (
      <div className="container mx-auto py-8 px-4 text-center">
        <h2 className="text-xl font-bold text-white mb-4">Fatura não encontrada</h2>
        <Link href="/view/pages/invoices" className="text-emerald-500 hover:underline text-sm">
          Voltar para a lista
        </Link>
      </div>
    );
        ...
        <div className="bg-zinc-800/50 p-5 sm:p-8 border-b border-zinc-800">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-xl sm:text-3xl font-bold mt-2 sm:mt-3 tracking-tighter">{invoice.name}</h1>
            </div>
            <div className="text-left sm:text-right border-t border-zinc-800 sm:border-0 pt-3 sm:pt-0">
              <p className="text-zinc-500 text-[9px] sm:text-xs uppercase tracking-widest font-bold">Valor</p>
          ...
          {invoice.description && (
            <div className="border-t border-zinc-800 pt-6 sm:pt-8">
              <div className="flex items-center gap-2 text-zinc-400 mb-2 sm:mb-3">
                <FileText size={16} />
                <span className="text-[10px] sm:text-sm font-bold uppercase tracking-wider">Descrição</span>
              </div>
...
function InfoItem({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
  return (
    <div className="flex items-center sm:items-start gap-3 sm:gap-4">
      ...
      <div>
        <p className="text-zinc-500 text-[9px] sm:text-[10px] uppercase tracking-widest font-bold mb-0.5 sm:mb-1">{label}</p>
...
function getStatusLabel(status: InvoiceStatus) {
  switch (status) {
    ...
    case InvoiceStatus.unpaid: return "Pendente";
    case InvoiceStatus.overdue: return "Atrasado";
    case InvoiceStatus.scheduled: return "Agendado";
